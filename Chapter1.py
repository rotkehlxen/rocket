...
def sqrt(x: float) -> float:
    ...
    return iterative_improve(guess_good_enough=lambda guess: good_enough(guess, x),
                             improve_guess=lambda guess:
...
def fixed_point(f: Callable) -> float:
    ...
    def improve_guess(f: Callable, guess: float) -> float:
        return f(guess)
    return iterative_improve(guess_good_enough=lambda guess: close_enough(f(guess), guess),
